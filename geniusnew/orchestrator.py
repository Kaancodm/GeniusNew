...
class Orchestrator:
    ...
    def dispatch(self, wire: Any, *, subject: str, job_id: str, policy: Policy,
        ...
        # one-time approval may be spent, so the admission stands whatever
        # happens next: a lost race for the job id below, or a process that
        # dies while the worker runs. If it cannot be passed on, nothing runs.
