...
| Der **Audit-Anker** läuft unter **demselben Betriebssystem-Nutzer** wie der Dienst | Ein Prozess dieses Nutzers kann ihn per Signal beenden — `verify` lehnt dann ab (fail closed), der Anker ist aber weg — oder seinen Speicher lesen, Antwortschlüssel eingeschlossen. Den Socket-Pfad auf einen eigenen Listener umzubiegen reicht nicht: jede Antwort ist über eine Nonce der Anfrage signiert. Ein eigener Nutzer ist Deployment | `tests/test_anchor_process.py::test_an_anchor_that_is_gone_fails_closed`, `::test_a_replaced_socket_is_refused_not_believed` |
